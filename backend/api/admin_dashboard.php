...
try {
    ...
    $qg = $pdo->prepare("SELECT DAY(created_at) AS dia, COUNT(*) AS total FROM vehicles WHERE MONTH(created_at) = ? AND YEAR(created_at) = ? GROUP BY DAY(created_at) ORDER BY dia ASC");
    ...
    foreach ($resultados as $row) { $datos_mensuales[$row['dia']] = (int)$row['total']; }
