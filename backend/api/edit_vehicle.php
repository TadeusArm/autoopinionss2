...
if ($metodo === 'GET') {
    ...
    if (!$vehicle) { echo json_encode(["success" => false, "message" => "Vehículo no encontrado."]); exit; }
    echo json_encode(["success" => true, "vehicle" => $vehicle, "user_id" => $user_id]);
    exit;
}
...
if ($metodo === 'POST') {
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] === 0) {
        ...
        if (!in_array($ext, $allowed)) { echo json_encode(["success" => false, "message" => "Formato no permitido."]); exit; }
        $image_name  = time() . "_" . $user_id . "." . $ext;
        $upload_path = __DIR__ . "/../../assets/img/vehicles/" . $image_name;
        if (!move_uploaded_file($_FILES['image']['tmp_name'], $upload_path)) { echo json_encode(["success" => false, "message" => "Error al subir imagen."]); exit; }
    }
    ...
    try {
        ...
        echo json_encode(["success" => true, "message" => "Vehículo actualizado.", "redirect" => "profile.html?id=" . $user_id]);
