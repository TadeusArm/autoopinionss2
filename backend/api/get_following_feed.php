...
try {
    $sql = "SELECT v.*, u.username,
            ...
            FROM vehicles v JOIN users u ON v.user_id = u.id JOIN follows f ON v.user_id = f.followed_id
            WHERE f.follower_id = ?";
    $params = [$user_id];
    ...
    foreach ($coches as &$c) {
        ...
        $c['image'] = !empty($c['image']) ? "/assets/img/vehicles/" . basename($c['image']) : null;
