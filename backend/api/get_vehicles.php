...
try {
    // 1. Obtener los vehículos con filtros
    ...
    foreach ($coches as &$c) {
        ...
        // Limpieza de ruta: nos aseguramos de que solo quede el nombre del archivo
        $img_db = $c['image'] ?? '';
        if (!empty($img_db)) {
            $nombre_archivo = basename($img_db);
            $c['image'] = "/assets/img/vehicles/" . $nombre_archivo;
        } else {
            $c['image'] = null;
        }
