...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (searchInput) {
        searchInput.addEventListener('input', () => {
            ...
            busquedaTimer = setTimeout(cargarFeed, 500);
        });
    }

    // Auto-recarga al cambiar cualquier selector
    document.querySelectorAll('select').forEach(select => {
        select.addEventListener('change', cargarFeed);
    });
    ...
    fetch(`/backend/api/get_following_feed.php?${queryParams.toString()}`)
        ...
        .then(data => {
            ...
            if (!data.vehicles || data.vehicles.length === 0) {
                timeline.innerHTML = `<p style="color:#94a3b8; text-align:center; margin-top:50px;">
                    No hay vehículos de tus seguidos. ¡Sigue a alguien para ver su contenido aquí!
                </p>`;
                return;
            }

            // Pintar tarjetas — misma estructura que index.js
